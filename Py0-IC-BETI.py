...
def _(mo):
    ...
    Alguns dos principais pontos que trazem **destaque para a Linguagem de Programação Python** são:
...
def _(mo):
    ...
    * `float`: número de ponto flutuante (decimal) positivo ou negativo, sempre se usa o ponto **.**  para separar as casas decimais
    * `complex`: representam números complexos, com uma "parte real" e uma "parte imaginária" de ponto flutuante e são escritos matematicamente no formato **a + bj**
...
def _(mo):
    ...
    **Nota0**: *iterable* é qualquer objeto Python capaz de retornar seus elementos um de cada vez, como em um laço "for".

    **Nota1**: Assim como em algumas linguagens de computação científica (Matlab, R...), as listas podem ser utilizadas para criar "vetores" e "matrizes".
    """)
    return
...
def _(mo):
    mo.md(r"""
    **Nota**: {:0.2f} é uma máscara de formatação usada para arredondar números de ponto flutuante para duas casas decimais e garantir que zeros à direita sejam exibidos.
    """)
    return
...
def _():
    # Com as chaves
    a3 = {"um": 1, "dois": 2, "tres": 3}
    b3 = dict(um=1, dois=2, tres=3)

    # Com a função dict()
    c3 = dict(zip(["um", "dois", "tres"], [1, 2, 3]))
    d3 = dict([("dois", 2), ("um", 1), ("tres", 3)])

    # Com a função zip() que "emparelha" duas listas
    e3 = dict({"tres": 3, "um": 1, "dois": 2})
